...
<div  id="gdds_head" class="flex">
    ...
    <li>
      <button class={activeTabClass('run')} on:click={() => onClickTab('run')}>
        ...
        >
          ...
          />
          <polygon
            points="10.5 10.5 15 15 18.5 11.5 15 8"
            fill="currentColor"
          />
        </svg>
        {$t('navbar.run')}
  ...
  <!-- <div class="dropdown dropdown-hover">
    ...
    <ul
      class="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-24"
    >
      {#each $locales as l}
        <li>
          <button
  ...
  <div class="hidden sm:flex flex-0 items-center justify-center p-2">
    <a href="http://gddongshi.com/" target="_blank">
      <svg
        class="hidden h-6 w-6 fill-current opacity-60 sm:inline-block"
